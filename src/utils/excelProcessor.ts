...
export function processRawRows(
  ...
  // 원본 상품명 추출 및 신규 품목 정제 목록 구축
  const rawNames = extractRawUniqueNames(rawRows, colMap.상품명, startRow);
  const newRawItems: string[] = [];

  rawNames.forEach(rawName => {
    const { isMatched } = extractOptionAndCleanName(rawName, productMap);
    if (!isMatched) {
      newRawItems.push(rawName);
    }
  });
